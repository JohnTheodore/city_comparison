...
def get_final_elections_dataframe():
  ...
  county_total_votes = pandas.DataFrame({'county_total_votes':
                                         elections_2020_dataframe.groupby('county_fips')['votes'].sum()})
  ...
  elections_2020_dataframe = elections_2020_dataframe[(elections_2020_dataframe['party'] == 'DEM') |
  ...
  # elections_2020_dataframe = elections_2020_dataframe.groupby('county_fips').apply(
  #   lambda x:
  #   {
  #     'county_fips': x['county_fips'],
  #     'dem_votes': x[x['party'] == 'DEM']['votes'],
  #     'rep_votes': x[x['party'] == 'REP']['votes'],
  #     'lib_votes': x[x['party'] == 'LIB']['votes']
  #   })
  # def dem_votes(party_votes):
  #   if party_votes.party == 'DEM':
  #     return party_votes.votes
  #   return 0

  # elections_2020_dataframe['dem_votes'] = elections_2020_dataframe.groupby('county_fips').apply(dem_votes).reset_index()
  elections_combined_party_rows_dataframe = elections_2020_dataframe.groupby('county_fips')[['party', 'county_vote_percent']].apply(lambda x: list(zip(x.party, x.county_vote_percent))).reset_index()
  ...
  elections_combined_party_rows_dataframe['county_dem_percent'] = elections_combined_party_rows_dataframe[0].apply(party_votes('DEM'))
  elections_combined_party_rows_dataframe['county_rep_percent'] = elections_combined_party_rows_dataframe[0].apply(party_votes('REP'))
  elections_combined_party_rows_dataframe['county_lib_percent'] = elections_combined_party_rows_dataframe[0].apply(party_votes('LIB'))
  elections_combined_party_rows_dataframe = elections_combined_party_rows_dataframe.drop(columns=[0])
